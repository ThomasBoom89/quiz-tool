...
func main() {
	...
	communication := quiz.NewCommunication(logger)

	userGroup := v1.Group("/user")
	quiz.NewUser(userGroup, logger, communication)
	adminGroup := v1.Group("/admin")
	quiz.NewAdmin(adminGroup, logger, communication)
	overviewGroup := v1.Group("/overview")
	quiz.NewOverview(overviewGroup, logger, communication)
